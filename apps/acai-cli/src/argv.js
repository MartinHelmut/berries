...
module.exports = function getConfiguredArgv(input) {
  const { argv } = yargs(input)
    ...
    .options({
      ...
      depth: {
        alias: "D",
        description: "Define how many commits in the past should be considered",
        type: "number"
      },
      files: {
        alias: "f",
    ...
    .example("$ $0", "If executed inside a git repository this is all you need")
    .example('$ $0 --cwd="path/to"', "If executed outside a git repository")
    .example('$ $0 --files "*.cpp" "**/*.h"', "To filter for specific files")
